...
function zip:value()
	...
	-- Each accumulator owned by this one returns a table of key-value
	-- pairs. We collect the metrics of the children accumulators in a
	-- single table, and output the result.
	for _, acc in pairs(self.accs) do
		...
		-- If the output of an accumulator only contains a single key-value
		-- pair with the key being the name of the accumulator, then we only
		-- collect the value.
