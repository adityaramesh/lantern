...
* May suffer from numerical issues in certain situations. If you suspect that this is happening to
  you, try `lantern.optimizers.adadelta`.
--]]

local adadelta_lm = torch.class('lantern.optimizer.adadelta_lm')
...
function adadelta_lm:update(batch)
	...
	if self.mom_type == lantern.momentum.none then
		...
		-- net contribution of zero in general. This may cause issues in certain situations.
