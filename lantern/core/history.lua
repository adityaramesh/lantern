...
function lantern.best_metrics(hist, mode, epochs)
	...
	local update_metrics = function(prev_best, cur)
		for k, v in pairs(cur) do
			--[[
			If `dir` is not defined for a metric, then we assume that it does not make
			sense to measure whether it has improved or not. So we avoid tracking it.
			--]]
			local dir = lantern.performance_metrics[k]
